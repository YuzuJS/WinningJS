...
module.exports = function Presenter(options) {
    ...
    function bindWinningEvents() {
        var elementsWithCommands = Array.prototype.slice.call(el.querySelectorAll("[data-winning-events]"));
        if (el.hasAttribute("data-winning-events")) {
            elementsWithCommands.push(el);
        }

        elementsWithCommands.forEach(listenForCommandsOn);
    }
