...
function ObservableVectorListDataAdapter(observableVector, mapping) {
    var that = this;
...
ObservableVectorListDataAdapter.prototype = {
    ...
    _itemFromIndex: function (index) {
        ...
        var data = this._mapping(this._observableVector[index]);
        return {
            data: data,
            key: index.toString()
        };
...
function ObservableVectorDataSource(observableVector, mapping) {
    this._baseDataSourceConstructor(new ObservableVectorListDataAdapter(observableVector, mapping));
}
