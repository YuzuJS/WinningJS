...
function ObservableVectorListDataAdapter(observableVector) {
    ...
    that._itemFromIndex = function (index) {
    ...
    that._notifyInsertedAtIndex = function (index) {
        ...
        that._notificationHandler.inserted(that._itemFromIndex(index), previousKey, nextKey, index);
    };
...
ObservableVectorListDataAdapter.prototype = {
    ...
    itemsFromIndex: function (index, countBefore, countAfter) {
        ...
        return WinJS.Promise.wrap({
            ...
            atEnd: end === length - 1,
            atStart: start === 0,
            items: _.range(start, end + 1).map(this._itemFromIndex),
            offset: countBefore,
            totalCount: length
        });
    },
