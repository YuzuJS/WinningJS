...
function ObservableVectorListDataAdapter(observableVector) {
    ...
    function itemFromIndex(index) {
        ...
        var data = that._observableVector[index];
        var item = {
            data: data,
            key: index.toString()
        };
    ...
    // This is factored out into a "private" method so that we can use it in `setNotificationHandler` later.
    that._notifyInsertedAtIndex = function (index) {
        var previousKey = index === 0 ? null : (index - 1).toString();
        var nextKey = index === that._observableVector.length - 1 ? null : (index + 1).toString();
        that._notificationHandler.inserted(itemFromIndex(index), previousKey, nextKey, index);
    };
...
ObservableVectorListDataAdapter.prototype = {
    ...
    itemsFromIndex: function (index, countBefore, countAfter) {
        ...
        return WinJS.Promise.wrap({
            ...
            atEnd: end === length - 1,
            atStart: start === 0,
            items: this._observableVector.slice(start, end + 1),
            offset: countBefore,
            totalCount: length
        });
    },
