...
describe "Create UI presenter", ->
    ...
    describe "process", ->
        ...
        describe "with markup continain data-winning-events attributes", ->
            presenter = null
            template = """
                       <section data-winning-events="mouseOver: commandA, itemInvoked: commandB" 
                                data-win-control="MyControls.Control"></section>
                       """
            ...
            it "should bind DOM events", (done) ->                              
                presenter.process().then((el) -> 
                    el.addEventListener.should.have.been.calledWith("mouseover", viewModel.commandA)
                ).should.notify(done)

            it "should bind winControl events", (done) ->
                presenter.process()

                presenter.winControl.then((winControl) ->
                    winControl.addEventListener.should.have.been.calledWith("iteminvoked", viewModel.commandB)
                ).should.notify(done)
